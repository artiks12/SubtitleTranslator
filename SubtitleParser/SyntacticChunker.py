...
from Configurations.Punctuations import closings
import stanza
        ...
        if len(temp) == 0:
            ...
            if not(last.end == token.start) or not(hasSpaces):
                if not(lastUpos == None):
                    if lastUpos == 'PUNCT' and token.upos == 'PUNCT':
                        pass
                    elif lastUpos in OtherPOS:
                        result.append(temp)
                        temp = ''
                    else:
                        if lastUpos in OpenPOS and token.upos in ClosedPOS:
                            ...
                            if not(lastNonTag.dep == token.id or lastNonTag.id == token.dep):
                                result.append(temp)
                                temp = ''
                        # elif lastUpos in ['NOUN','PROPN'] and token.upos == 'VERB':
                        #     result.append(temp)
                        #     temp = ''  
...
if __name__ == "__main__":
    ...
    chunks = GetSyntachticChunksAsStrings(test1.tokens,True)
    for chunk in chunks:
        print(chunk)
